...
use std::{fs::{File, OpenOptions}, os::unix::prelude::FromRawFd};
...
impl<'a> framebuffer::FramebufferBase<'a> for Framebuffer<'a> {
        ...
        let device = if true {
            if let Some(lib) = &*LIBRM2FB_CLIENT {
                unsafe {
                    ...
                    let open_func: libloading::Symbol<unsafe extern "C" fn(fd: *const libc::c_char, flags: libc::c_int, mode: libc::mode_t) -> libc::c_int> = lib.get(b"open").unwrap();
    ...
    fn put_var_screeninfo(device: &File, var_screen_info: &mut VarScreeninfo) -> bool {
        let result = unsafe { auto_ioctl!(device.as_raw_fd(), FBIOPUT_VSCREENINFO, var_screen_info) };
        result == 0
    }
